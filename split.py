...
from utils import GDALSaver, GDALLoader, LAND_NO_DATA_VAL, SPA_NO_DATA_VAL

# TODO 修复 utils import
# TODO 去掉 region.tif
...
def split_blocks(
    ...
    def is_legal(region: np.ndarray):
        return np.any(region == 1)
    ...
    region = rasters["region"]

    blocks_to_save = []

    num_rows, num_cols = region.shape[0], region.shape[1]
    if crop:
        existing_block = set()
...
def split(cfg: DictConfig):
    ...
    spatial_vars = cfg["spatial_vars"] + ["region"]
    ...
    gdal_saver = GDALSaver(raster_paths["region"])
    split_blocks(
        rasters,
